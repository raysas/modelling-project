...
;  let w biased / ( biased - unbiased )  ;;-- rayane: bias, we'll use this in the probability (didnt work)
  ...
  ;; --rayane: trying new r walks
;  let r_walk compute_k * ( ( 1 - w ) * unbiased + w * biased )  ;;-- trying new wighted proba
;  let r_walk compute_k * w
;  let r_walk compute_k * ( ( biased + ln biased ) / ( unbiased ) )
;  print (word "current r_Walk is " r_walk)
  ...
  ask ICs [
    ...
    ;;-- rayane note: the problem is its always gonna be biased
...
;    ;; -- rayane note: swapping from r_walk < r to r < r_walk AND adding an else for random mouvement (note: should random be towards tumor?)
;    ifelse  r < r_walk
...
;    [set chosen_normal_cell one-of N_neighbors] ;; move randomly
  ...
  ask NICs with [mode = 0][
    ...
    if is-IC? change_neighbor [
      ...
      ;;die             ;;--rayane: if asked to die here wont move inside the next ask (try to print a messae in it)
      ask patch IC-x IC-y [
